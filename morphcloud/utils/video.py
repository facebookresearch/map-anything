...
def preprocess_inputs(
    ...
    """
    Preprocess input_frames by determining optimal aspect ratio and resizing all images and multi-modal inputs.
    ...
    Args:
        input_frames (list): List of dictionaries containing frame data. Each frame can contain:
    ...
    Returns:
        list: List of processed frame dictionaries with resized images and multi-modal inputs
    """
    # Validate resize_mode and size parameter requirements
    valid_resize_modes = ["fixed_mapping", "longest_side", "square", "fixed_size"]
    if resize_mode not in valid_resize_modes:
    ...
    if not input_frames:
        raise ValueError("input_frames cannot be empty")

    # First pass: Extract all images and collect aspect ratios
    aspect_ratios = []
    for frame_idx, frame in enumerate(input_frames):
        if "img" not in frame:
            if verbose:
                print(
                    f"Warning: frame {frame_idx} has no 'img' key, skipping for aspect ratio calculation"
                )
            continue
        ...
        if isinstance(img, torch.Tensor):
            ...
            if img.ndim == 3 and img.shape[2] == 3:
                ...
                raise ValueError(
                    f"Expected tensor shape (H, W, 3) for img in frame {frame_idx}, got {img.shape}"
                )
            ...
            if img.ndim == 3 and img.shape[2] == 3:
                ...
                raise ValueError(
                    f"Expected array shape (H, W, 3) for img in frame {frame_idx}, got {img.shape}"
                )
        else:
            raise ValueError(f"Unsupported image type in frame {frame_idx}: {type(img)}")

        aspect_ratios.append(W / H)

    if not aspect_ratios:
        raise ValueError("No valid images found in input_frames")
    ...
    if verbose:
        print(
            f"Using target resolution {target_size[0]}x{target_size[1]} (W x H) for all frames"
        )
    ...
    def to_pil_image(img, frame_idx):
    ...
    def to_numpy(data, expected_shape, name, frame_idx):
        ...
        if not isinstance(data, np.ndarray):
            raise ValueError(
                f"Expected tensor or array for {name} in frame {frame_idx}, got {type(data)}"
            )

        if data.shape != expected_shape and expected_shape is not None:
            raise ValueError(
                f"Expected shape {expected_shape} for {name} in frame {frame_idx}, got {data.shape}"
            )
    ...
    for frame_idx, frame in enumerate(input_frames):
        ...
        if "img" not in frame:
            raise ValueError(f"frame {frame_idx} missing required 'img' key")

        img = to_pil_image(frame["img"], frame_idx)
        ...
        # Handle depth_z
        if "depth_z" in frame:
            depthmap = to_numpy(frame["depth_z"], None, "depth_z", frame_idx)
            if depthmap.ndim != 2:
                raise ValueError(
                    f"Expected shape (H, W) for depth_z in frame {frame_idx}, got {depthmap.shape}"
                )

        # Enforce that only one of intrinsics and ray_directions is provided
        has_intrinsics = "intrinsics" in frame
        has_ray_directions = "ray_directions" in frame

        if has_intrinsics and has_ray_directions:
            raise ValueError(
                f"frame {frame_idx} cannot have both 'intrinsics' and 'ray_directions'. "
                "Please provide only one as they are redundant (ray_directions can be used to recover intrinsics)."
            )

        # Handle intrinsics
        if has_intrinsics:
            intrinsics = to_numpy(frame["intrinsics"], (3, 3), "intrinsics", frame_idx)

        # Handle ray_directions by recovering intrinsics from them
        if has_ray_directions:
            ray_dirs = to_numpy(
                frame["ray_directions"], None, "ray_directions", frame_idx
            )
            if ray_dirs.ndim != 3 or ray_dirs.shape[2] != 3:
                raise ValueError(
                    f"Expected shape (H, W, 3) for ray_directions in frame {frame_idx}, got {ray_dirs.shape}"
                )
        ...
        processed_frame["img"] = ImgNorm(resized_img)[
            None
        ]  # Add batch dimension like load_images
        processed_frame["data_norm_type"] = [norm_type]  # Add normalization type
        result_idx += 1

        # Depth is next if provided - add batch dimension
        if depthmap is not None:
            processed_frame["depth_z"] = torch.from_numpy(results[result_idx])[None]
            result_idx += 1

        # Intrinsics is next if provided - add batch dimension
        if intrinsics is not None:
            processed_frame["intrinsics"] = torch.from_numpy(results[result_idx])[None]
            result_idx += 1

        # Handle camera_poses with batch dimension if present
        if "camera_poses" in frame:
            ...
            if isinstance(camera_poses, tuple):
                ...
                processed_frame["camera_poses"] = (quats_batched, trans_batched)
            else:
                # Matrix format - add batch dimension
                if isinstance(camera_poses, torch.Tensor):
                    processed_frame["camera_poses"] = camera_poses[None]
                elif isinstance(camera_poses, np.ndarray):
                    processed_frame["camera_poses"] = torch.from_numpy(camera_poses)[
                        None
                    ]
                else:
                    raise ValueError(
        ...
        processed_frame["frame_idx"] = frame_idx

        processed_frames.append(processed_frame)

        if verbose:
            print(f"Processed frame {frame_idx} with keys: {list(processed_frame.keys())}")
