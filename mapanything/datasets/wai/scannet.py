...
"""
ScanNetV2 Dataset using WAI format data.
"""
...
class ScanNetWAI(BaseDataset):
    """
    ScanNetV2 dataset containing large diversity of indoor scenes.
    """
    ...
    def _load_data(self):
        ...
        split_metadata_path = os.path.join(
            ...
            self.split,
            f"scannetv2_scene_list_{self.split}.npy",
        )
    ...
    def _get_views(self, sampled_idx, num_views_to_sample, resolution):
        ...
        for view_index in view_indices:
            ...
            view_data = load_frame(
                scene_root,
                view_file_name,
                modalities=["image", "depth", "semantic_label"],
                scene_meta=scene_meta,
            )

            # Convert necessary data to numpy
            image = view_data["image"]
            depthmap = view_data["depth"]
            semantic_label = view_data["semantic_label"]
            intrinsics = view_data["intrinsics"]

            scale = torch.tensor([depthmap.shape[1] / image.shape[2], depthmap.shape[0] / image.shape[1]], device=intrinsics.device)
            intrinsics[:2] = intrinsics[:2] * scale.unsqueeze(1)

            image = resize(image, depthmap.shape[:2], InterpolationMode.BILINEAR)
            image = image.permute(1, 2, 0).numpy()
            image = (image * 255).astype(np.uint8)

            semantic_label = resize(semantic_label.unsqueeze(0), depthmap.shape[:2], InterpolationMode.NEAREST).squeeze(0)

            depthmap = depthmap.numpy().astype(np.float32)
            semantic_label = semantic_label.numpy().astype(np.int32)
            ...
            image, depthmap, intrinsics, additional_quantities = self._crop_resize_if_necessary(
                image=image,
                resolution=resolution,
                depthmap=depthmap,
                intrinsics=intrinsics,
                additional_quantities=[semantic_label],
            )
            semantic_label = additional_quantities[0]
...
def get_parser():
    ...
    parser.add_argument(
        "-rd", "--root_dir", default="/ai4rl/fsx/xrtech/data/scannetpp", type=str
    )
    parser.add_argument(
        "-dmd",
        "--dataset_metadata_dir",
