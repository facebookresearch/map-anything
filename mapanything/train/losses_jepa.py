...
from mapanything.train.losses import Criterion, MultiLoss


class JEPALoss(MultiLoss):
    def __init__(self):
        super().__init__()
        self.criterion = nn.MSELoss()

    def get_name(self):
        return f"JEPALoss({self.criterion._get_name()})"
    
    def compute_loss(self, batch, ctx_out, tgt_out, **kwargs):
        num_views = len(batch)
        total_loss = 0
        ...
        for i, (ctx, tgt) in enumerate(zip(ctx_out, tgt_out)):
            pred = ctx["predicter_feature"]
            target = tgt["adapter_features"][-1]
            target_mask = ctx["target_mask"]

            if target_mask:
                loss = self.criterion(pred, target.detach())
                loss_details[self_name][f"{self_name}_view{i}"] = loss
        
        loss_details[self_name] = sum(loss_details[self_name].values()) / len(loss_details[self_name].values())
        total_loss += loss_details[self_name]
        
        return total_loss, loss_details


class LinearProbeLoss(MultiLoss):
    ...
    def get_name(self):
        return f"LinearProbeLoss({self.criterion._get_name()})"

    def compute_loss(self, batch, full_out, **kwargs):
        total_loss = 0
        self_name = self.get_name()
        loss_details = {
            self_name: {},
        ...
        for i, view in enumerate(batch):
            predicted_logits = full_out[i]["probe_logits"]
            target = view["semantic_mask"]
            target = (target - 1).long()

            if (target != -1).sum() == 0:
                continue

            loss = self.criterion(predicted_logits, target)
            loss_details[self_name][f"view{i}"] = loss
            loss_details['IoU'][f"view{i}"] = get_mIoU(predicted_logits, target)

        # for i in range(num_views):
        #     loss_details[f"{self_name}_view{i}"] = loss_details[self_name][f"view{i}"]

        if len(loss_details[self_name]) == 0:
            return predicted_logits.sum() * 0, {}

        loss_details[self_name] = sum(loss_details[self_name].values()) / len(loss_details[self_name].values())
        loss_details['IoU'] = sum(loss_details['IoU'].values()) / len(loss_details['IoU'].values())
        total_loss += loss_details[self_name]
...
@torch.no_grad()
def get_mIoU(predicted_logits: torch.Tensor, target: torch.Tensor, ignore_index=-1):
    """
    Compute mean Intersection over Union (mIoU) for semantic segmentation.
    
    Args:
        predicted_logits: (B, C, H, W) tensor of logits
        target: (B, H, W) tensor of ground truth labels
        ignore_index: label value to ignore during computation (default: -1)
    
    Returns:
        mIoU score as a Python float
    """
    # Get predicted class labels
    preds = predicted_logits.argmax(dim=1)  # (B, H, W)
    ...
    # Flatten and filter valid pixels only
    preds_valid = preds[valid_mask]      # (N,)
    target_valid = target[valid_mask]    # (N,)
    
    # Handle empty valid region
    if preds_valid.numel() == 0:
        return 0.0
    
    # One-hot encode predictions and targets
    preds_onehot = nn.functional.one_hot(preds_valid, num_classes=num_classes).float()   # (N, C)
    target_onehot = nn.functional.one_hot(target_valid, num_classes=num_classes).float() # (N, C)
    
    # Compute intersection and union per class
    intersection = (preds_onehot * target_onehot).sum(dim=0)  # (C,)
    union = ((preds_onehot + target_onehot) > 0).float().sum(dim=0)  # (C,)
    
    # Compute IoU only for classes present in ground truth (standard practice)
    iou_per_class = []
    for cls in range(num_classes):
        ...
        if target_onehot[:, cls].sum() > 0:
            if union[cls] > 0:
                iou_per_class.append(intersection[cls].item() / union[cls].item())
    
    # Return mean IoU across valid classes
...
# class ConfJEPALoss(Criterion, MultiLoss):ß
#     """
#     Loss on JEPA self supervised learning with confidence.
#     """
...
#     def forward(self, batch, ctx_out, tgt_out):
