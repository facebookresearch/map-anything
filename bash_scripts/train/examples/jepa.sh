...
torchrun --nproc_per_node ${NUM_GPUS} \
    ...
    train_params=jepa_online_probe \
    train_params.epochs=10 \
    train_params.warmup_epochs=1 \
    train_params.keep_freq=200 \
    train_params.max_num_of_imgs_per_gpu=4 \
    hydra.run.dir='${root_experiments_dir}/mapanything/training_examples/jepa_probe_scannet/${now:%Y-%m-%d_%H-%M-%S}'
